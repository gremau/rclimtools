...
get_rolling_CV <- function(ts_in, window, positivize=FALSE, plots=FALSE,
    ...
    ts_rmean <- zoo::rollapply(ts_in, window, mean, align='right')
    ts_rsd <- zoo::rollapply(ts_in, window, sd, align='right')
    if (positivize==TRUE){
        # Calculate an adjusted ts (>0 for calculating CV) and get rolling mean
        # and standard deviation
        ts_adj <- ts_in - min(ts_in, na.rm=TRUE)
        ts_adj_rmean <- zoo::rollapply(ts_adj, window, mean, align='right')
        ts_adj_rsd <- zoo::rollapply(ts_adj, window, sd, align='right')
        # Calculate CV
